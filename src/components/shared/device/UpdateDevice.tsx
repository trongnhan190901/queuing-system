...
const UpdateDevice = ({ deviceData, deviceId }: UpdateDeviceProp) => {
    ...
    const handleRemoveService = (index: number) => {
        if (deviceData) {
            const updatedServiceUse: string[] =
                deviceData.serviceUse?.split(',') || [];
            updatedServiceUse.splice(index, 1);
            const updatedServiceUseString = updatedServiceUse.join(',');

            setServiceUse(updatedServiceUseString);

            deviceData.serviceUse = updatedServiceUseString;
        }
    };

    const serviceUseList = deviceData?.serviceUse
        ?.split(',')
        .map((service, index) => (
            <span
                className="px-3 flex w-fit text-[17px] font-bold rounded-xl bg-orange-400 text-white"
                key={index}
            >
                {service}
                <XMarkIcon
    ...
    const showDetailDeviceComponent = () => {
    ...
    return (
        <>
                ...
                <>
                    ...
                    <div className="w-[95%] ml-14 h-[580px] pb-24 rounded-3xl drop-shadow-xl shadow-xl bg-white">
                            ...
                            <div className="mt-12 full-size">
                                                    ...
                                                    {({ open }) => (
                                                        <>
                                                            ...
                                                            >
                                                                <Listbox.Options
                                                                    static
                                                                    className="absolute w-[96%] mt-3 text-[16px] bg-white border border-gray-300 rounded-xl shadow-lg overflow-hidden z-50 focus:outline-none"
                                                                >
                                                                    {deviceType.map(
                                                                        (
                                                                            option,
                                ...
                                <div className="w-full text-[16px] mt-6">
                                    <div className="flex font-primary flex-col space-y-2">
                                        ...
                                        <div className="border-2 border-orange-400 rounded-xl h-36 w-[98%]">
                                            <div className="mx-4 my-3 flex space-x-6 ">
                                                {serviceUseList}
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="w-full mt-12 text-2xl justify-center flex space-x-6">
                            <button
                                onClick={showDetailDeviceComponent}
            ...
            {isParentVisible && <DeviceContainer />}
        </>
    );
};
